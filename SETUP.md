...
- MCP-инструменты (`mem_search`, `mem_probe`, `mem_get_turn`, `mem_get_session`, `mem_stats`, `mem_audit_tail`), доступные в любом MCP-совместимом клиенте. `mem_probe(term)` — точный FTS-счётчик (coverage-oracle: сколько, где, когда); `mem_search` — ранжирующий поиск с нечёткими совпадениями. Каждый вызов логируется в `anamnesis_audit`; `mem_audit_tail` возвращает хвост для интроспекции.
