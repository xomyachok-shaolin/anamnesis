...
- **Ежедневные бэкапы**: `~/anamnestic-backups/claude-mem-YYYYMMDD-HHMMSS.tar.gz`
...
PYTHONPATH=$PWD ~/.claude-mem/semantic-env/bin/python -m anamnestic.cli restore \
    ~/anamnestic-backups/claude-mem-LATEST.tar.gz

# 5. Запустить миграции (no-op если БД восстановлена полностью)
PYTHONPATH=$PWD ~/.claude-mem/semantic-env/bin/python -m anamnestic.db
...
systemctl --user enable --now anamnestic-sync.timer anamnestic-backup.timer

# 8. Зарегистрировать MCP server в Claude Code
claude mcp add anamnestic ~/.claude-mem/semantic-env/bin/python \
    -e PYTHONPATH=$PWD -- -m anamnestic.daemon.mcp_server
```
...
ls -lt ~/anamnestic-backups/ | head

# Остановить активные операции
systemctl --user stop anamnestic-sync.timer
fuser -k 37777/tcp  # claude-mem worker
# Если был запущен MCP server — Claude Code перезапустит сам

anamnestic restore ~/anamnestic-backups/claude-mem-<stamp>.tar.gz
...
anamnestic verify
anamnestic eval    # smoke test
systemctl --user start anamnestic-sync.timer
```
...
1. Попытка `recover`:
       ...
       anamnestic verify   # против /tmp/recovered.db вручную
...
anamnestic sync   # всё проиндексируется заново (несколько минут)
```

## Как проверить, что восстановление прошло успешно

```bash
anamnestic status         # должны быть счётчики, healthy=true
anamnestic verify         # все чеки ok
anamnestic eval           # 100% pass
anamnestic search "any известный запрос"
```
...
- MCP регистрация в Claude Code (`claude mcp add anamnestic ...`).
