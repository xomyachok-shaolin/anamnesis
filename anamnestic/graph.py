...
def build_edges(limit: int | None = None) -> dict:
    ...
    query = """
        ...
        FROM anamnestic_entities ae
        JOIN historical_turns ht ON ht.id = ae.turn_id
        LEFT JOIN anamnestic_graph_state gs
    ...
    for row in sessions:
        ...
        for a, b in combinations(values, 2):
            ...
            if a > b:
                a, b = b, a
            conn.execute(
                """INSERT INTO anamnestic_entity_edges (entity_a, entity_b, weight, sessions)
                   VALUES (?, ?, 1, ?)
        ...
        conn.execute(
            "INSERT OR IGNORE INTO anamnestic_graph_state(content_session_id) VALUES (?)",
            (sid,),
        )
        processed += 1
