...
def archive_old_turns(
    ...
    rows = conn.execute(
        ...
        JOIN anamnestic_summary_state ss
    ...
    for row in rows:
        conn.execute(
            """
            INSERT OR IGNORE INTO anamnestic_archived_turns
    ...
    if ids_to_delete:
        ...
        # Clean embed state
        conn.execute(
            f"DELETE FROM anamnestic_embed_state WHERE turn_id IN ({placeholders})",
            ids_to_delete,
        )
