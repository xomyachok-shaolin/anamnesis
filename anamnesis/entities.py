...
Extracted entities go into `anamnesis_entities` and enable scoped queries
    ...
    try:
        while True:
            rows = conn.execute(
                ...
                LEFT JOIN anamnesis_entity_state es ON es.turn_id = ht.id
            ...
            for row in rows:
                ...
                if entities:
                    conn.executemany(
                        "INSERT OR IGNORE INTO anamnesis_entities"
                        "(turn_id, entity_type, value) VALUES (?, ?, ?)",
                        [(tid, etype, val) for etype, val in entities],
                    )
                    added += len(entities)
                conn.execute(
                    "INSERT OR IGNORE INTO anamnesis_entity_state(turn_id) VALUES (?)",
                    (tid,),
                )
                processed += 1
            conn.commit()
...
def stats() -> dict:
    conn = connect()
    try:
        total = conn.execute("SELECT COUNT(*) FROM anamnesis_entities").fetchone()[0]
        by_type = {
            r[0]: r[1]
            for r in conn.execute(
                "SELECT entity_type, COUNT(*) FROM anamnesis_entities GROUP BY entity_type"
            ).fetchall()
        }
        processed = conn.execute("SELECT COUNT(*) FROM anamnesis_entity_state").fetchone()[0]
