...
Records per-peer attempt/success/error in `anamnesis_peer_state`.

Peer config — first match wins:

  1. CLI flag `--peer host[,host2]`
  2. Env `ANAMNESIS_PEERS` — colon- or comma-separated `user@host` or `host`
...
from anamnesis.config import CC_ROOT, CODEX_ROOT, DATA_DIR
...
def discover_peers(cli_peers: list[str] | None = None) -> list[str]:
    if cli_peers:
        ...
        env = os.environ.get("ANAMNESIS_PEERS", "").replace(":", ",").split(",")
...
def _record_attempt(peer: str, success: bool, dur: float, err: str,
    ...
    try:
        conn.execute(
            """
            INSERT INTO anamnesis_peer_state
        ...
        if success:
            conn.execute(
                "UPDATE anamnesis_peer_state SET last_success_at = datetime('now') "
                "WHERE peer = ?",
                (peer,),
            )
        conn.commit()
