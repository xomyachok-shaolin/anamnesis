...
`anamnesis threads` CLI. Results go into anamnesis_session_threads.
"""
...
def compute(gap_sec: int = GAP_THRESHOLD_SEC) -> dict:
    ...
    conn = connect()
    try:
        conn.execute("DELETE FROM anamnesis_session_threads")
        ...
        conn.executemany(
            "INSERT INTO anamnesis_session_threads"
        ...
        solo_threads = conn.execute(
            """
            SELECT COUNT(DISTINCT thread_id) FROM anamnesis_session_threads
...
def get_thread(session_id: str) -> list[dict]:
    ...
    try:
        tid = conn.execute(
            "SELECT thread_id FROM anamnesis_session_threads WHERE session_id = ?",
            (session_id,),
        ).fetchone()
        if not tid:
            return []
        ...
        rows = conn.execute(
            ...
            FROM anamnesis_session_threads t
...
def stats() -> dict:
    ...
    try:
        ...
        total_sessions = conn.execute(
            "SELECT COUNT(*) FROM anamnesis_session_threads"
        ).fetchone()[0]
        multi = conn.execute(
            """
            SELECT COUNT(*) FROM (
                SELECT thread_id FROM anamnesis_session_threads
                GROUP BY thread_id HAVING COUNT(*) > 1
            )
            """
        ).fetchone()[0]
        longest = conn.execute(
            ...
            FROM anamnesis_session_threads
