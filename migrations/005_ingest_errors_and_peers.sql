...
CREATE TABLE IF NOT EXISTS anamnestic_ingest_errors (
...
CREATE INDEX IF NOT EXISTS idx_ingest_errors_at        ON anamnestic_ingest_errors(at DESC);
CREATE INDEX IF NOT EXISTS idx_ingest_errors_source    ON anamnestic_ingest_errors(source);
CREATE INDEX IF NOT EXISTS idx_ingest_errors_unresolved
    ON anamnestic_ingest_errors(path)
    WHERE resolved_at IS NULL;

-- Cross-host sync state — per-peer last-success / last-failure metadata.
CREATE TABLE IF NOT EXISTS anamnestic_peer_state (
